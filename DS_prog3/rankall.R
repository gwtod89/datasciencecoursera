...
rankall <- function(typeOfIll = char(), ranking) {
	...
	## Refind Data 
	outcome[,3:5]<- sapply(outcome[, c(3:5)], as.numeric); 
	postSelection <- split(outcome, outcome$State)

	## Find Best hospital 
	tempVar <- colnames(outcome)

	for (n in 3:5){
		if(typeOfIll == tempVar[n]){
			
			for (i in 1:length(postSelection))
				postSelection[i]<- postSelection[[order(postSelection[[i]][n])]] 
